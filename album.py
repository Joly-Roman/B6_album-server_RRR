...
def find(artist):
    # Находит альбомы по заданному артисту
    session = connect_db()
...
def find_album(album_data):
    ...
    session = connect_db()
    existing_album = session.query(Album).filter(Album.artist == artist).filter(Album.year == year).filter(Album.genre == genre).filter(Album.album == album).first()
